...
def main():
    ...
    with torch.no_grad():
        for dataset, dataloader in dataloaders:
            ...
            n = 0
            for b, (x, _) in tqdm(enumerate(dataloader, desc='DataLoader Progress')):
                x = x.to(device)
                ...
                # Regular ELBO
                for i in tqdm(range(args.iw_samples_elbo),
                              leave=False,
                              desc='L(X) Progress'):
                ...
                # L>k bound
                for i in tqdm(range(args.iw_samples_Lk),
                              leave=False,
                              desc='L>K(X) Progress'):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_eval_examples", type=int, default=float("inf"),
                        help="cap on the number of examples to use")
